function drawCameraCentricObject(cameraX, cameraY){
    ...
    this.drawGrid = function()
    {
        ...
        for(var x = -game.height/40; x <= game.height/40; x++){
            ctx.beginPath();
            ctx.moveTo(gameXToCanvasX(-game.width/2 + game.gameWidth/2 - this.cameraX + 400*game.x), gameYToCanvasY(x*20 + game.gameHeight/2 + this.cameraY - 400*game.y));
            ctx.lineTo(gameXToCanvasX(game.width/2 + game.gameWidth/2 - this.cameraX + 400*game.x), gameYToCanvasY(x*20 + game.gameHeight/2 + this.cameraY - 400*game.y));
            ctx.stroke();
            ctx.closePath();
        }
        //vertical lines
        for(var x = -game.width/40; x <= game.width/40; x++){   
            ctx.beginPath();
            ctx.moveTo(gameXToCanvasX(x*20 + game.gameWidth/2 - this.cameraX + 400*game.x), gameYToCanvasY(-game.height/2 + game.gameHeight/2 + this.cameraY - 400*game.y));
            ctx.lineTo(gameXToCanvasX(x*20 + game.gameWidth/2 - this.cameraX + 400*game.x), gameYToCanvasY(game.height/2 + game.gameHeight/2 + this.cameraY - 400*game.y));
            ctx.stroke();
            ctx.closePath();
        }
    }
